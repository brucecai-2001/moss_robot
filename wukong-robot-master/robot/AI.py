...
from robot.sdk import SparkApi
...
class SPARKRobot(AbstractRobot):
    ...
    def __init__(self):
        self.appid = "35122103"     #填写控制台中获取的 APPID 信息
        self.api_secret = "ZDdjZWIyMmFhZjNjY2Q2YjkzNGQ5ZWE2"   #填写控制台中获取的 APISecret 信息
        self.api_key ="ecd2035adc843cae2ab6909b39994aa5"    #填写控制台中获取的 APIKey 信息
    ...
    def buildPrompt(self,role,content):
        jsoncon = {}
        jsoncon["role"] = role
    ...
    def chat(self, message):
        # user query
        self.buildPrompt("user",message)

        # send to server
        SparkApi.answer =""

        wsParam = SparkApi.Ws_Param(self.appid, self.api_key, self.api_secret, self.Spark_url)
        websocket.enableTrace(False)
        wsUrl = wsParam.create_url()
        ws = websocket.WebSocketApp(wsUrl, on_message=SparkApi.on_message, on_error=SparkApi.on_error, on_close=SparkApi.on_close, on_open=SparkApi.on_open)
        ws.appid = self.appid
        ws.question = self.prompt
        ws.domain = self.domain
        ws.run_forever(sslopt={"cert_reqs":ssl.CERT_NONE})

        # get server response
        self.buildPrompt("moss",SparkApi.answer)
